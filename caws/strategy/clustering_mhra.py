...
class MHRA(Strategy):
    ...
    def preprocess(self, tasks):
        ...
        for i, task in enumerate(tasks):
            for j, endpoint in enumerate(self.endpoints):
                task_runtime, task_energy = self.predictor.predict(endpoint, task)
        ...
        tasks_by_energy = zip(task_energies, task)
        return list(tasks_by_runtime) list(tasks_by_energy), task_embeddings

    def cluster(self, tasks_by_energy, task_embeddings, threshold=1e6):
        ...
        for i in range(children.shape[0]):
            n0 = children[i,0]
            n1 = children[i,1]
            if (n0 in clusters) and (n1 in clusters) and (clusters[n0][0] < threshold or clusters[n1][0] < threshold):
    ...
    def schedule(self, tasks):
        tasks_by_runtime, tasks_by_energy, task_embeddings = self.preprocess(tasks)
        tasks_by_energy = self.cluster(tasks_by_energy, task_embeddings)
        
        task_runtimes = {task: time for (time, task) in tasks_by_runtime}
        tasks_by_runtime = [(sum(task_runtime[t] for t in cluster), cluster) for _, cluster in tasks_by_energy]
